...
def test_run_install_root_production_restores_env_permissions_for_app_user(tmp_path: Path, monkeypatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    (repo_root / ".env.example").write_text(
        "APP_ENV=production\n"
    ...
    assert ["chown", "agentos:agentos", env_path.as_posix()] in commands
    assert ["chmod", "600", env_path.as_posix()] in commands


def test_run_install_production_sync_excludes_runtime_env_file(tmp_path: Path, monkeypatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    (repo_root / ".env.example").write_text(
        "APP_ENV=production\n"
    ...
    target = tmp_path / "target"
    target.mkdir()
    (target / ".env").write_text("ENABLED_GATEWAYS=cli\n", encoding="utf-8")
    commands: list[list[str]] = []
    ...
    sync_command = next(command for command in commands if command and command[0] == "rsync")

    assert exit_code == 0
    assert sync_command[:9] == ["rsync", "-a", "--delete", "--exclude", ".git", "--exclude", ".venv", "--exclude", ".env"]
